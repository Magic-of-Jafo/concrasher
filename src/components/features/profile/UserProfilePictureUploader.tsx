...
// Themed dialog paper so the crop/remove modals (which render in a portal, out
// of the profile surface) still read as House Lights.
...
const UserProfilePictureUploader: React.FC<UserProfilePictureUploaderProps> = ({ currentImageUrl, onImageUpdate, user }) => {
    ...
    // and the user zooms in from there. No pre-squaring/padding.
    ...
    const handleFileSelect = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        img.onload = async () => {
            ...
            try {
                // Load the original as-is; fitToCropBox handles the initial fit.
    ...
    const createCroppedImage = useCallback(async (imageSrc: string, cropArea: Area): Promise<Blob> => {
        return new Promise((resolve, reject) => {
            ...
            image.addEventListener('load', () => {
                ...
                // aspect={1} keeps cropArea square; draw it to fill the 400x400 canvas.
                ctx.drawImage(
                    image,
    ...
    const handleCropConfirm = useCallback(async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                setError(result.message || 'Failed to save image to your profile.');
            }
        } catch (error) {
            console.error(error);
    ...
    return (
        <Box ref={containerRef} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1.5 }}>
            {/* Avatar — themed frame, with a placeholder icon when empty. */}
            <Box
                sx={{
                    width: AVATAR_SIZE,
                    height: AVATAR_SIZE,
            ...
            >
                ...
                <DialogActions>
                    <Button onClick={() => setShowRemoveDialog(false)} disabled={isProcessing} sx={{ color: 'var(--cc-muted)', textTransform: 'none' }}>
                        Cancel
                    </Button>
