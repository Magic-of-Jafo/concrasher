...
interface BasicInfoTabProps {
  /** Initial data to populate the form, typically when editing an existing convention. */
  initialData?: Partial<BasicInfoFormData>;
  /** Callback function invoked when any form field value changes. */
  onFormChange: (field: keyof BasicInfoFormData, value: any) => void;
  /** An object map of field names to error messages for displaying validation errors. */
...
export const BasicInfoTab: React.FC<BasicInfoTabProps> = ({
  ...
  const [formData, setFormData] = useState<Partial<BasicInfoFormData>>(() => {
    ...
    return {
      ...defaultFormData,
      ...resolvedInitialData,
      country: resolvedInitialData.country !== undefined && resolvedInitialData.country !== null 
               ? resolvedInitialData.country 
               : defaultFormData.country,
      descriptionShort: resolvedInitialData.descriptionShort ?? defaultFormData.descriptionShort,
      descriptionMain: resolvedInitialData.descriptionMain ?? defaultFormData.descriptionMain,
      seriesId: resolvedInitialData.seriesId ?? defaultFormData.seriesId,
    };
  });

  /**
   * Effect to synchronize formData with initialData prop changes.
   * This is important if the initialData can change after the component has mounted
   * (e.g., if parent data loads asynchronously).
   */
  useEffect(() => {
    const resolvedInitialData = initialData || {};
    setFormData({
  ...
  }, [initialData]); // Use initialData directly as dependency

  /** Handles changes for standard text input fields and updates the slug if the name field changes. */
  const handleTextChange = (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = event.target;
    let newSlug = formData.slug;

    if (name === 'name') {
      newSlug = slugify(value, { lower: true, strict: true, remove: /[*+~.()'"!:@]/g });
      setFormData(prev => ({ ...prev, [name]: value, slug: newSlug }));
      onFormChange(name as keyof BasicInfoFormData, value);
  ...
  /** Handles changes for DatePicker components. */
  const handleDateChange = (field: 'startDate' | 'endDate') => (date: Date | null) => {
    setFormData(prev => ({ ...prev, [field]: date }));
    onFormChange(field, date);
  };

  /** Handles changes for Switch components (isOneDayEvent, isTBD). */
  const handleSwitchChange = (field: 'isOneDayEvent' | 'isTBD') => (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => {
      ...
      if (field === 'isOneDayEvent' && checked) {
        newState.endDate = newState.startDate; // Set endDate to startDate
      } else if (field === 'isOneDayEvent' && !checked) {
        // Optional: Clear endDate when toggled off, or let user re-select. For now, let's not clear it automatically.
      }
      return newState;
    });
    onFormChange(field, checked);
    if (field === 'isOneDayEvent' && checked && formData.startDate) {
      onFormChange('endDate', formData.startDate); // Also notify parent form about endDate change
    }
  };

  /** Handles content changes from the TinyMCE editor. */
                  ...
                  <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                    <Box sx={{ width: 250 }}>
                      <DatePicker
                        ...
                        disabled={!!formData.isTBD}
                        slotProps={{ textField: { error: !!errors?.startDate, helperText: errors?.startDate, sx:{width: '100%'} } }}
                      />
                    </Box>
                    {!formData.isOneDayEvent && (
                      <Box sx={{ width: 250 }}>
                        <DatePicker
                          ...
                          disabled={!!formData.isTBD}
                          slotProps={{ textField: { error: !!errors?.endDate, helperText: errors?.endDate, sx:{width: '100%'} } }}
                        />
                      </Box>
                    )}
                  </Box>
          ...
          <Paper elevation={1} sx={{ p: 2, width: '100%' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Autocomplete
                ...
                value={COUNTRIES.find(opt => opt.label === formData.country) || formData.country || null}
                onChange={(_, newValue: any) => {
                  ...
                  const newFormDataState = { ...formData, country: countryString };
                ...
                onInputChange={(_, newInputValue, reason) => {
                  if (reason === 'input') {
                    setFormData(prev => {
                      const updatedFormData = { ...prev, country: newInputValue };
