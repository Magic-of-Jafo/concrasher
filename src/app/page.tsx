...
export default async function Home() {
  ...
  async function handleSignIn() {
    "use server";
    // Redirects to the default sign-in page, which will show Credentials form
  ...
  return (
    <div className="grid grid-rows-[auto_1fr_auto] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <header className="flex flex-col items-center gap-4">
        <h1 className="text-2xl font-semibold">NextAuth.js v4 Test</h1>
        {session?.user ? (
          <div className="text-center">
            <p>Signed in as {session.user.email}</p>
            <p>User ID: {session.user.id}</p>
            <form action={handleSignOut}>
              <button type="submit" className="mt-2 px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600">
                Sign Out
              </button>
            </form>
          </div>
